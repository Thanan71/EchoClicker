...
globalThis.GameCurrency = {
    ...
    spendEnergy(amount) {
        if (this._state.energy < amount) return false;
        this._state.energy -= amount;
        return true;
    },

    spendLinks(amount) {
        if (this._state.links < amount) return false;
    ...
    getClickPower() {
        ...
        if (this._state.boosts.energy) power *= 2;
    ...
    getPassiveIncome() {
        ...
        if (this._state.boosts.energy) income *= 2;
        return income;
    },
