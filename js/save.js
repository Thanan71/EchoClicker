...
export const SaveSystem = {
    ...
    load() {
        try {
            ...
            if (json) {
                ...
                for (const oldKey of this.OLD_KEYS) {
                    ...
                    if (json) {
                        data = JSON.parse(json);
                        sourceKey = oldKey;
                        console.log(`Migration: sauvegarde trouvee dans ${oldKey}`);
                        break;
                    }
                }
            }
            ...
            if (result && sourceKey !== this.KEY) {
                ...
                console.log(`Migration: sauvegarde migree de ${sourceKey} vers ${this.KEY}`);
            }
    ...
    migrateSave(data) {
        ...
        if (data.v === 3) {
            console.log('Migration v3 -> v4: ajout des quetes et inventaire');
            if (!migrated.s) migrated.s = {};
            if (!migrated.s.quests) migrated.s.quests = null;
            if (!migrated.s.inventory) migrated.s.inventory = [];
            ...
            if (migrated.s.regions) {
                ...
                console.log('Migration: regions fusionnees avec les nouvelles zones');
            }
            migrated.v = 4;
        }
        ...
        if (data.v === 2) {
            ...
            if (!migrated.s) migrated.s = {};
            if (!migrated.s.mine) migrated.s.mine = null;
            if (!migrated.s.hatchery) migrated.s.hatchery = null;
            return this.migrateSave({ ...migrated, v: 3 });
        }

        // Migration de la version 1 vers 2
        if (data.v === 1) {
            console.log('Migration v1 -> v2: ajout des boosts et regions');
            if (!migrated.s) migrated.s = {};
            if (!migrated.s.boosts) migrated.s.boosts = {};
            if (!migrated.s.regions) {
    ...
    loadFromData(data, skipMigration = false) {
        try {
            ...
            return SaveSerializer.deserialize(data, Game.state, {
                Mine,
                Hatchery,
                questSystem,
                NarrativeSystem,
    ...
    importFromData(data) {
        try {
            if (!data) {
                console.error('Import: donnees invalides');
                return false;
            }
            ...
            if (result) {
                this.save();
                console.log('Import: sauvegarde importee avec succes');
            }
