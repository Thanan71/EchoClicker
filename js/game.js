...
export const Game = Object.assign({}, GameState, GameParty, GameCurrency, GameRoutes, {
  ...
  click(event) {
    ...
    if (event) {
      UI.spawnParticle(event, `+${Utils.formatNumber(power)}`);
    }
  },
  ...
  buyItem(item) {
    ...
    if (item.currency === 'shards') {
      this.state.shards -= item.price;
    }
    if (item.amount && item.id.startsWith('l')) {
      this.addLinks(item.amount);
    }
    if (item.id === 'evo') {
      this.state.crystals++;
    }
    ...
    if (item.id === 'potion') {
      for (const e of this.getAllEchoes()) {
        e.fullHeal();
      }
    }
    if (item.duration) {
      this.state.boosts[item.type] = { endTime: Date.now() + item.duration * 1000 };
    }
  ...
  setupEvents() {
    ...
    document
      .getElementById('btn-capture-combat')
      .addEventListener('click', () => Combat.attemptCapture());
    ...
    document
      .getElementById('btn-logbook')
      .addEventListener('click', () => NarrativeSystem.showLogbook());
    ...
    document.getElementById('game-main').addEventListener('click', (e) => {
      ...
      if (document.getElementById('tab-capture').classList.contains('active')) {
        return;
      }
      this.click(e);
    });
  ...
  setupEventBus() {
    EventBus.on(GAME_EVENTS.ECHO_LEVELED_UP, ({ echo }) => {
      if (echo.level > this.state.maxLevel) {
        this.state.maxLevel = echo.level;
      }
