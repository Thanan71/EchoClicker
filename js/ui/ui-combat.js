...
export const UICombat = {
  ...
  _computeTeamHash(party) {
    const activeUid = Combat.activeEcho?.uid || 'none';
    return `${party
      .map(
        (e) => `${e.uid}:${e.level}:${Math.floor(e.hp)}:${Math.floor(e.xp)}:${e.isAlive() ? 1 : 0}`,
      )
      .join('|')}@${activeUid}`;
  },

  _buildEchoMemberHTML(echo) {
  ...
  _updateTeamPanel() {
    ...
    if (!party || party.length === 0) {
      if (this._lastTeamHash !== 'empty') {
        teamList.innerHTML = '<p class="team-empty">Aucun Écho dans l\'équipe</p>';
        this._lastTeamHash = 'empty';
      }
      return;
    }

    const teamHash = this._computeTeamHash(party);
    if (teamHash === this._lastTeamHash) {
      return;
    }
    this._lastTeamHash = teamHash;

    teamList.innerHTML = party.map((echo) => this._buildEchoMemberHTML(echo)).join('');
  },
