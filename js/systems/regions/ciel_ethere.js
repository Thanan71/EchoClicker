...
(() => {
  ...
  function drawSkyPath(ctx, ra, rb, unlocked, time, a, b) {
    ctx.beginPath();
    ctx.moveTo(ra.x, ra.y);
    const cp1x = ra.x + (rb.x - ra.x) * 0.3 + Math.sin(time + a) * 12;
    const cp1y = ra.y + (rb.y - ra.y) * 0.3 - 20;
    const cp2x = ra.x + (rb.x - ra.x) * 0.7 + Math.cos(time + b) * 12;
  ...
  function drawSkyPaths(map) {
    ...
    for (const [a, b] of [
      ...
      drawSkyPath(ctx, ra, rb, ul, map.time, a, b);
    }
  }
