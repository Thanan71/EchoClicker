...
(function() {
    function drawSkyBackground(map) {
        ...
        g.addColorStop(0, '#1a1a40'); g.addColorStop(0.3, '#2a2a60');
        g.addColorStop(0.6, '#1a2040'); g.addColorStop(1, '#151535');
        ctx.fillStyle = g; ctx.fillRect(0, 0, w, h);
        for (let i = 0; i < 60; i++) {
            const x=(i*137.5)%w, y=(i*73.7)%(h*0.6);
            const twinkle=0.4+Math.sin(map.time*2.5+i*0.8)*0.4, size=1+(i%3)*0.5;
            ctx.beginPath(); ctx.arc(x,y,size,0,Math.PI*2);
            ctx.fillStyle='rgba(255,255,200,'+twinkle+')'; ctx.fill();
            if (i%5===0) {
                ctx.strokeStyle='rgba(255,255,200,'+(twinkle*0.3)+')'; ctx.lineWidth=0.5;
    ...
    function drawClouds(map) {
        const ctx = map.ctx;
        [{x:0.15,y:0.3,s:1.2},{x:0.5,y:0.25,s:1.5},{x:0.8,y:0.35,s:1.0},{x:0.35,y:0.55,s:0.8},{x:0.65,y:0.5,s:1.1}].forEach(c => {
            const cx=c.x*map.width+Math.sin(map.time*0.3+c.x*10)*15;
            const cy=c.y*map.height, s=30*c.s;
            for (let i = 0; i < 4; i++) {
                const ox=(i-1.5)*s*0.4, oy=Math.sin(i*1.5)*s*0.15, r=s*(0.3+Math.sin(i*2)*0.1);
                ctx.beginPath(); ctx.arc(cx+ox,cy+oy,r,0,Math.PI*2);
                const cg=ctx.createRadialGradient(cx+ox,cy+oy,0,cx+ox,cy+oy,r);
                cg.addColorStop(0,'rgba(200,210,255,0.15)'); cg.addColorStop(0.7,'rgba(150,170,220,0.08)');
                cg.addColorStop(1,'transparent'); ctx.fillStyle=cg; ctx.fill();
            }
            const aura=ctx.createRadialGradient(cx,cy,0,cx,cy,s*0.8);
            aura.addColorStop(0,'rgba(200,200,255,0.05)'); aura.addColorStop(1,'transparent');
            ctx.fillStyle=aura; ctx.fillRect(cx-s,cy-s,s*2,s*2);
        });
    }
    ...
    function drawLightRays(map) {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            ctx.beginPath(); ctx.moveTo(x-15,0); ctx.lineTo(x+30,map.height*0.7);
            ctx.lineTo(x-30,map.height*0.7); ctx.closePath();
            const rg=ctx.createLinearGradient(x,0,x,map.height*0.7);
            rg.addColorStop(0,'rgba(255,240,200,'+alpha+')'); rg.addColorStop(1,'transparent');
    ...
    function drawEtherealParticles(map) {
        ...
        for (let i = 0; i < 25; i++) {
            ...
            const alpha=0.3+Math.sin(map.time*3+i)*0.2, size=2+(i%3);
            ctx.beginPath(); ctx.arc(x,y,size,0,Math.PI*2);
    ...
    function drawSkyPaths(map) {
        ...
        [[0,1],[1,2],[0,3],[3,4],[2,3],[1,4]].forEach(([a,b]) => {
            ...
            ctx.lineWidth=ul?3:2; ctx.setLineDash(ul?[]:[6,6]); ctx.stroke(); ctx.setLineDash([]);
        });
    }
