...
(function() {
    function drawTree(map, x, y, size) {
        ...
        for (let i = 0; i < 3; i++) {
            ctx.beginPath();
            ctx.arc(x + sway + (i - 1) * size * 0.25, y - size * 0.5 - i * size * 0.15, size * (0.4 - i * 0.05), 0, Math.PI * 2);
    ...
    function drawForestTrees(map) {
        ...
        trees.forEach(t => { drawTree(map, t.x * map.width, t.y * map.height, 40 * t.s); });
    }

    function drawGlowMushrooms(map) {
        const ctx = map.ctx;
        const mushrooms = [{ x: 0.3, y: 0.4 }, { x: 0.6, y: 0.35 }, { x: 0.45, y: 0.6 }, { x: 0.7, y: 0.55 }];
        mushrooms.forEach(m => {
            ...
            ctx.fillStyle = '#8b4513'; ctx.fillRect(mx - 2, my, 4, 8);
            ctx.beginPath(); ctx.arc(mx, my, 6, Math.PI, 0);
            ctx.fillStyle = `rgba(200, 255, 150, ${pulse})`; ctx.fill();
        });
    }
    ...
    function drawForestPaths(map) {
        ...
        const curveOffsets = [
            { dx: 15, dy: -10 }, { dx: -20, dy: 15 }, { dx: 10, dy: 20 },
            { dx: -15, dy: -15 }, { dx: 20, dy: 10 }, { dx: -10, dy: -20 }
        ];
        paths.forEach(([a, b], i) => {
            ...
            ctx.setLineDash(unlocked ? [] : [8, 8]); ctx.stroke(); ctx.setLineDash([]);
            if (unlocked) {
                for (let t = 0.2; t < 1; t += 0.2) {
                    const px = ra.x + (rb.x - ra.x) * t + (Math.sin(t * 10) * 5);
                    const py = ra.y + (rb.y - ra.y) * t + (Math.cos(t * 10) * 5);
                    ctx.beginPath(); ctx.arc(px, py, 2, 0, Math.PI * 2);
                    ctx.fillStyle = 'rgba(139, 119, 101, 0.4)'; ctx.fill();
                }
            }
        });
    }
