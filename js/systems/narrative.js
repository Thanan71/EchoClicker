...
export const NarrativeSystem = {
  ...
  triggerGuideDialogue(trigger) {
    ...
    dialogue = guide.dialogues.tips.find(
      (d) => d.trigger === trigger && !this.state.seenDialogues.has(d.id),
    );
    if (dialogue) {
      this.showGuideDialogue(dialogue);
    ...
    dialogue = guide.dialogues.events.find(
      (d) => d.trigger === trigger && !this.state.seenDialogues.has(d.id),
    );
    if (dialogue) {
      this.showGuideDialogue(dialogue);
  ...
  checkUnlockCondition(condition, stats, state) {
    switch (condition.type) {
      ...
      case 'boss_defeated': {
        ...
        return regionBoss?.bossDefeated;
      }
  ...
  playCinematic(cinematicId) {
    if (this.state.seenCinematics.has(cinematicId)) {
      return;
    }
  ...
  showCinematicScene(cinematic, sceneIndex) {
    ...
    if (nextBtn) {
      nextBtn.addEventListener('click', () =>
        this.nextCinematicScene(cinematic.title, sceneIndex + 1),
      );
    }
  },
  ...
  nextCinematicScene(cinematicTitle, nextIndex) {
    ...
    const cinematic = Object.values(NARRATIVE_DATA.cinematics).find(
      (c) => c.title === cinematicTitle,
    );
    if (cinematic) {
  ...
  showLogbook() {
    ...
    for (const tab of document.querySelectorAll('.logbook-tab')) {
      tab.addEventListener('click', (e) => {
        for (const t of document.querySelectorAll('.logbook-tab')) {
