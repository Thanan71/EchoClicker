...
export const MapCore = {
  ...
  navigateToRegion(regionId) {
    if (regionId === this.currentRegionId || this.transitioningFrom) {
      return;
    }
    ...
    for (const btn of document.querySelectorAll('.map-nav-btn')) {
  ...
  handleMouseMove(e) {
    ...
    this.canvas.style.cursor = found?.route.unlocked ? 'pointer' : 'default';
  },
  ...
  getKillsNeededForRoute(routeIndex) {
    ...
    if (!prevRoute || !prevRoute.unlocked) {
      return -1;
    }
    return GAME_CONFIG.KILLS_FOR_ROUTE;
  },
